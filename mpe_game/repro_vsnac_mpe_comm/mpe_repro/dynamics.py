...
class AircraftDynamics:
    ...
    def _aero_terms_batch(
        ...
        vb = x[:, 4]
        vh = x[:, 5]
        speed = np.sqrt(va * va + vb * vb + vh * vh)
    ...
    def f_batch(self, x: np.ndarray) -> np.ndarray:
        thrust, drag, lift, moment, alpha, _ = self._aero_terms_batch(x)
        out = np.zeros_like(x)
        out[:, :3] = x[:, 3:6]
        out[:, 3] = (thrust * np.cos(alpha) - drag) / self.params.m
        out[:, 4] = (lift + thrust * np.sin(alpha)) / self.params.m
    ...
    def control_effectiveness_batch(self, x: np.ndarray) -> np.ndarray:
        _, _, _, _, _, g33 = self._aero_terms_batch(x)
        return g33

    def g_transpose_dot_batch(self, x: np.ndarray, grad: np.ndarray) -> np.ndarray:
        g33 = self.control_effectiveness_batch(x)
