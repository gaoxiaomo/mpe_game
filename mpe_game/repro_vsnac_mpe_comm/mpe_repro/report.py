...
def eval_summary(result: EvalResult, comm_mode: str = "full") -> Dict[str, Any]:
    min_d_min = float(np.min(result.d_min_history)) if result.d_min_history.size > 0 else None
    mean_formation_error = (
        float(np.mean(result.formation_errors)) if result.formation_errors.size > 0 else None
    )
    return {
